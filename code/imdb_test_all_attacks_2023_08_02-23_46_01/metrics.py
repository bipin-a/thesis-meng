...
def get_transferability(
    ...
    adv_dataset_paths = [
        f'{model}_/{attack}/glue_cola.csv'
    ...
    file_name = f"metrics/transferability_results.csv"
...
def get_bertscore(original, perturbed):
    bertscore_result = bertscore.compute(predictions=perturbed,
            references=original,
            lang="en")
    return bertscore_result

def get_adv_examples_metrics(experiment_name, inference_model_names, attack_names):
    '''
    ...
    for model in inference_model_names:
        ...
        for attack in attack_names:
            ...
            bertscore_result = get_bertscore(original, perturbed)
